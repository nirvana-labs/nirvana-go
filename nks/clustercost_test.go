...
func TestClusterCostNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.NKS.Clusters.Cost.New(context.TODO(), nks.ClusterCostNewParams{
		Autoscaling:       true,
...
func TestClusterCostUpdateWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
