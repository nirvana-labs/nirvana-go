...
// BorrowRepayService contains methods and other services that help with
// interacting with the Nirvana Labs API.
//
// Note, unlike clients, this service does not read variables from the environment
// automatically. You should not instantiate this service directly, and instead use
// the [NewBorrowRepayService] method instead.
type BorrowRepayService struct {
	Options []option.RequestOption
}

// NewBorrowRepayService generates a new service that applies the given options to
// each request. These options are applied after the parent client's options (if
// there is one), and before any request-specific options.
func NewBorrowRepayService(opts ...option.RequestOption) (r BorrowRepayService) {
	r = BorrowRepayService{}
	r.Options = opts
	return
}
...
func (r BorrowRepayNewParams) MarshalJSON() (data []byte, err error) {
	type shadow BorrowRepayNewParams
